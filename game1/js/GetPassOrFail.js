...
var pressTimesArray=[0,0,0,0,0];//指创建长度为X的数组 
var playerPressSpeed = 0;
function game1_setting_init(config){
    pressTimesArray=new Array((parseInt(config.preferTime)+1));
    for(var i = 0; i < pressTimesArray.length;i++)		//获得玩家最近5秒的按键次数
	{
		pressTimesArray[i] = 0;
	}
}
...
function GetPassOrFail(clickTimes)
{
	if(keepPressTimes<10)		//获得10次参考按键次数
	{
		referencePressTimes += clickTimes;
		keepPressTimes++;
	}
	...
	pressTimesArray[pressTimesArray.length - 1] = clickTimes;
	//clickTimes = 0;
